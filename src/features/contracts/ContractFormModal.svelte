    ...
    <div class="min-h-0 flex-1 overflow-y-auto px-0 mt-0">
      <form on:submit|preventDefault={onSubmit} class="p-0 space-y-4">
        <div class="mt-5 grid grid-cols-1 gap-x-4 sm:grid-cols-2">
          {#each visibleFields as field}
            <label
              class="mb-4 {fieldConfig.longTextFields.has(field)
                ? 'sm:col-span-2'
                : ''}"
            >
              {field}
              ...
              {:else}
                <input
                  class="mt-1.5"
                  type={fieldConfig.dateFields.has(field)
                    ? "date"
                    : fieldConfig.numericFields.has(field)
                      ? "number"
                      : "text"}
                  step={fieldConfig.numericFields.has(field)
                    ? "any"
                    : undefined}
                  bind:value={formValues[field]}
                />
      ...
      <div class="flex gap-2 border-t border-slate-200 pt-2 mt-2">
        ...
        <Button
          ...
          type="submit"
          disabled={saving}
          on:click={onSubmit}>{saving ? "Đang lưu..." : "Lưu hồ sơ"}</Button
        >
      </div>
    </div>
  </div>
